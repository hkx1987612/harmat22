...
- The existing public `/epitesi-naplo/` page now leads with the official August 2026 construction video `HMgnTfeuQYM`. `wp-mu-plugins/zz-harmat-construction-progress-video.php` version `1.0.1` injects one responsive main-video section without creating a duplicate page or changing the homepage video.
- Initial page load serves the tracked 1280x720 local poster `assets/construction/harmat-epitesi-naplo-2026-08.jpg` and no YouTube iframe. A visitor click loads the privacy-enhanced `youtube-nocookie.com` player, preserving hosting bandwidth and first-load comfort. The page has updated Hungarian SEO descriptions, the local social image and one linked `VideoObject` for the 1:31 video.
- Private rollback backups: `/home/harmath2/codex-backups/construction-video-20260829-054905` and `/home/harmath2/codex-backups/construction-video-date-fix-20260829-055812`. The supplemental guarded update aligned the Schema `uploadDate` with YouTube's exact `2026-08-28` publication date. Temporary and final PHP files passed lint; live plugin and poster hashes match GitHub, WordPress core checksums, every database table and WP-Cron passed, no staging files remain, and the existing root error log predates deployment.
- Regression: the 145-page, 124-property and 572-resource scan returned zero issues. Desktop and 390-pixel mobile checks passed the construction page plus the homepage, gallery, apartment search, A1-1-L2, both virtual selectors, the A1 building selector and contact page with one H1, no public Chinese text, no horizontal overflow and intact media. The A1-1-L2 quote modal retained its exact apartment data, privacy requirement and five approved lead sources. All 36 existing offer leads remained and no test inquiry was submitted.
