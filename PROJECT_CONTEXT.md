...
- Local assistant `0.4.0` now reads the same published property summary helper used by the public site through the read-only MU adapter `zz-harmat-assistant-live-data.php` version `1.0.0`. All 124 apartments and their PDF links are present; the four corrected sales areas remain 47.83 m2. Hidden prices are masked and are not treated as cheap offers. No property metadata, offer calculation, CRM record, homepage or advertising setting was changed.
- Bounded room/budget/area/building/floor/outdoor selection criteria survive follow-up questions within the current page. FAQ questions retain those criteria, and a localized new-search action clears them. This is the existing deterministic assistant, not a new external LLM. New ask requests send structured criteria, not raw conversation history; the existing voluntary handoff behavior remains unchanged.
- HU/ZH/EN recommendation-card offer actions reuse the existing unified quote modal with the chosen apartment. Pending/error replies are localized, duplicate requests are guarded, and a 15-second timeout retains a sales-contact fallback. Quote submission and pricing code are unchanged.
- Backup: `/home/harmath2/codex-backups/assistant-live-data-20260910-183744`. Temporary and final PHP lint, exact live hashes, cache clearing and WordPress boot passed. Tests: 39 isolated assertions, desktop/390px three-language assistant-to-quote flows and simulated failures, nine key pages on both viewports, plus 145 public pages / 124 properties / 588 resources with zero scan findings. All 44 offer leads and root error-log size/time remained unchanged; no test inquiry was submitted. See `server-config/maintenance/2026-09-10-assistant-live-data.md` for rollback and limitations.
- Previous stable tag: `stable-2026-09-10-home-native-video-current`
...
- Successful rollback backup: `/home/harmath2/codex-backups/construction-gallery-20260829-063049`. Temporary and final PHP files passed lint; all 32 live gallery hashes match GitHub, cache was cleared, WordPress core checksums, every database table and WP-Cron passed, no staging files remain, and the existing root error log predates deployment. Two earlier guarded attempts rolled back completely after the HTML counter included a JavaScript selector; no partial gallery or plugin change remained.
- Regression: the 145-page, 124-property and 588-resource scan returned zero issues. Desktop and 390-pixel mobile checks passed gallery layout, high-resolution lightbox navigation and the click-to-load video plus the homepage, gallery, apartment search, A1-1-L2, both virtual selectors, the A1 building selector and contact page. The A1-1-L2 quote modal retained its exact apartment data, privacy requirement and five approved lead sources. All 36 existing offer leads remained and no test inquiry or conversion was submitted.
- Previous stable tag: `stable-2026-08-29-construction-video-current`
- Previous stable tag: `stable-2026-08-27-a2-room-name-current`
- Data-only correction: property `5205` / `A2-3-L1`, room `A2-3-L1/08` now reads `Szoba`, retaining `18.34 m2`. The actual living room `/04` remains `Nappali`, `22.68 m2`. Both Elementor HTML widget `4520a1d` and the fallback `post_content` were corrected; no theme, MU plugin, public JavaScript, CSS, PDF or floor-plan image was changed.
- Private rollback backup: `/home/harmath2/codex-backups/a2-room-name-20260827-154745/property-5205-before.json`. The version-controlled, WP-CLI-only repair has exact source-hash, apartment, widget, room-code and area guards; no recurring frontend hook was added. See `server-config/maintenance/2026-08-27-a2-room-name.md`.
- Verification: 11 focused tests passed locally and on the server; temporary and final PHP files passed lint, dry run passed, and a repeat run reported already corrected. Structural comparison found exactly one changed Elementor HTML value; all other target post fields and metadata matched before cache purge. All 124 property bodies were compared and only `5205` changed. All 34 offer leads and error-log size/timestamp were unchanged. The 145-page, 124-property and 573-resource regression scan returned zero issues; Chrome confirmed the corrected row, retained living-room label, loaded floor-plan image, no desktop overflow and the offer modal with correct property/price and five source choices. No test inquiry or conversion was submitted.
- Previous stable tag: `stable-2026-08-27-confirmed-leads-current`
...
- Google Ads account `133-638-0278`, campaign `Harmat Lakópark` (`23887334395`), retains `HUF 5,000/day` and Maximize conversions. Its upgraded asset group (`6716281376`) has 12 images, one logo, two videos, 15 headlines, four long headlines, five descriptions and six sitelinks. After the short-video save, Google showed the group as under review/not serving and ad strength pending evaluation; saved is not the same as approved or delivering.
- Ads-only refinement on August 27 saved eight relevant non-brand buying-intent search themes, for nine including the original brand theme. After the user completed Google's phone identity confirmation in Chrome, the 15-second landscape short `https://www.youtube.com/watch?v=bf7SNOlbTYQ` was successfully saved to the existing group. Reopening the editor verified both video IDs, unchanged copy/images/sitelinks, all nine search themes and the unchanged budget. The original `kmAg_ki-yYY` video remains attached. A portrait template failed preview checks and remains unpublished. See `server-config/maintenance/2026-08-27-google-ads-refinement.md` for exact themes, rollback and verification. The website stable tag is unchanged.
