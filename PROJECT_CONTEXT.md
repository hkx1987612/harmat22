...
- Ads-only refinement on August 27 saved eight relevant non-brand buying-intent search themes, for nine including the original brand theme; reopening the signals editor verified all nine. One 15-second landscape short was uploaded as `https://www.youtube.com/watch?v=bf7SNOlbTYQ`, but saving its association to the existing group requires Google's identity confirmation and is not yet verified. A portrait template failed preview checks and remains unpublished. See `server-config/maintenance/2026-08-27-google-ads-refinement.md` for exact themes, rollback and continuation. The website stable tag is unchanged.
