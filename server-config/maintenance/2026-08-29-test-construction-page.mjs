...
try {
  for (const test of [
    ...
    assert(audit.description.includes('2026. augusztusi'), `${test.name}: description is stale.`);
    ...
    assert(audit.layout.playerBottom <= audit.layout.listTop, `${test.name}: video overlaps the project log.`);
